...
// fixed in rc.1.
_semver.default.gte(reactVersion,'17.0.0-rc.1');const distDir=_path.default.join(dir,config.distDir);const babelLoader=config.experimental.turboMode?require.resolve('./babel/loader/index'):'next-babel-loader';const defaultLoaders={babel:{loader:babelLoader,options:{isServer,distDir,pagesDir,cwd:dir,// Webpack 5 has a built-in loader cache
...
version:`${"10.1.4-canary.11"}|${configVars}`,cacheDirectory:_path.default.join(distDir,'cache','webpack')};// Adds `next.config.js` as a buildDependency when custom webpack config is provided
